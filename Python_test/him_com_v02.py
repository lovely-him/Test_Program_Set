...
class Win_UI():
    ...
    def __init__(self):
        ...
        # 从文件中加载UI定义
        self.ui = QUiLoader().load('./Python_test/ui/untitled_6.ui')
    ...
    def read_img_timer_connect(self):
        ...
        for x in range(128):                                                # 反转图片
            for y in range(32):
                show_img[x][y] = com_img[31-y][x]                           # 180°的反转
    ...
    def read_img_horizontalSlider(self):
        ...
        self.progress_bar = int(self.read_img_num*100/len(self.read_text))      # 计算进度条的进度

        old_progress_bar = self.ui.horizontalSlider.value()                     # 获取值

        # 如果当前进度条比原本进度条还大5个点，就认为跳变了
        if old_progress_bar >= self.progress_bar+5*100/len(self.read_text): 
            self.read_img_num = int(old_progress_bar*len(self.read_text)/100)
            self.progress_bar = int(self.read_img_num*100/len(self.read_text))  # 重新计算
        
        self.signals.horizontalSlider.emit(self.progress_bar)                   # 设置进度条进度
...
if __name__ == '__main__':
    app = QApplication([])
    # 加载 icon
    app.setWindowIcon(QIcon('./Python_test/ui/ico_0.png'))
    com_ui = Win_UI()
    com_ui.ui.show()
    app.exec_()
